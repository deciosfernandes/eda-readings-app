...
## 2026-06-01 - [Providing actionable context with consumption deltas]
**Learning:** Displaying raw meter readings in a history list is informative, but calculating and displaying the difference (delta) between consecutive readings provides immediate actionable context about energy usage trends. Integrating this delta into screen reader semantics ensures that this improved context is accessible to all users without adding visual clutter.
**Action:** Pre-calculate consumption deltas during data loading and display them as secondary badges while also appending them to the accessibility labels of the list items.
