...
## 2026-06-10 - Comprehensive Hardening of Development Proxies (DoS & Info Leakage)
**Vulnerability:** The development proxy lacked request body size limits, was susceptible to path traversal bypasses via dot segments, and leaked raw exception details.
**Learning:** Simple prefix checks on `request.uri.path` can be bypassed by browsers or attackers using dot segments (e.g., `/../`) which are normalized differently by upstream servers. Additionally, enforcing DoS limits requires checking both `Content-Length` and the actual stream bytes to handle chunked transfers where length is unknown.
**Prevention:** Always use `uri.normalizePath()` before validating path prefixes. Enforce strict byte limits on request streams using `request.map` or similar transformers to catch oversized payloads in real-time. Mask all internal errors with generic messages (e.g., "Internal Server Error") to prevent leaking environment or upstream details.
