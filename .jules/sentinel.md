...
## 2026-06-10 - Hardening Proxies against Normalization Bypasses and Header Duplication
**Vulnerability:** The development proxy relied on initial string-based path checks that could be bypassed by  segments normalized later by the URI constructor, leaked error details to the client, and used inconsistent header forwarding.
**Learning:** Checking a path string *before* normalization is insufficient; attackers can use encoded or relative segments to reach unauthorized endpoints. Additionally, using  for security headers can lead to multiple conflicting headers if the upstream also provides them, potentially confusing the browser or weakening the security policy.
**Prevention:** Always perform a redundant path validation on the *final* resolved  object after all components have been combined and normalized. Use  for security-critical headers (, , etc.) to enforce a single, authoritative policy, and return generic error messages to the client to prevent information leakage.

## 2026-06-10 - Hardening Proxies against Normalization Bypasses and Header Duplication
**Vulnerability:** The development proxy relied on initial string-based path checks that could be bypassed by `..` segments normalized later by the URI constructor, leaked error details to the client, and used inconsistent header forwarding.
**Learning:** Checking a path string *before* normalization is insufficient; attackers can use encoded or relative segments to reach unauthorized endpoints. Additionally, using `headers.add()` for security headers can lead to multiple conflicting headers if the upstream also provides them, potentially confusing the browser or weakening the security policy.
**Prevention:** Always perform a redundant path validation on the *final* resolved `Uri` object after all components have been combined and normalized. Use `headers.set()` for security-critical headers (e.g., `X-Frame-Options`, `X-Content-Type-Options`) to enforce a single, authoritative policy, and return generic error messages to the client to prevent information leakage.
