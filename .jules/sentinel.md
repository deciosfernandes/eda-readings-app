...
## 2026-06-11 - Hardening Development Proxy against Path Traversal, DoS, and Information Leakage
**Vulnerability:** The development proxy lacked path normalization, request body size limits, and masked error handling, making it susceptible to traversal bypasses, memory exhaustion (DoS), and sensitive data exposure.
**Learning:** Checking prefixes on raw URI paths can be bypassed by encoded traversal segments (e.g., `..`). Proxies must also enforce resource limits (like 1MB payloads) on the incoming stream to handle chunked transfers where `Content-Length` is missing. Finally, raw exceptions should never be returned to the client as they may leak backend architecture or dependency details.
**Prevention:** Always normalize URI paths before prefix validation. Enforce strict payload size limits using both header checks and stream transformers. Implement generic error responses (`Internal Server Error`) to mask underlying implementation details from potential attackers.
