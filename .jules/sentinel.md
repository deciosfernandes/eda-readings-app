...
## 2026-06-10 - Preventing Proxy DoS via Robust Size Limits and Generic Error Handling
**Vulnerability:** The development proxy lacked request body size limits, exposing it to local DoS, and leaked internal proxy errors in response bodies.
**Learning:** Relying solely on `Content-Length` for size validation is insufficient as it can be bypassed using chunked transfer encoding (where `Content-Length` is -1). Robust protection requires a stream transformer to count actual received bytes. Additionally, exposing detailed error messages (e.g., `Proxy error: ...`) can leak internal architecture or upstream server state.
**Prevention:** Implement "Defense in Depth" for size limits by checking both `Content-Length` and using a byte-counting stream transformer. Always return generic error messages to clients while keeping detailed logs for server-side debugging. Use `headers.set()` for security headers to ensure consistency and prevent duplication.
