...
## 2026-06-11 - Robust DoS Protection and Path Traversal Prevention in Proxies
**Vulnerability:** Proxies often lack strict request body size limits and are susceptible to path traversal bypasses using normalized segments like `..` or percent-encoded characters.
**Learning:** Checking the `Content-Length` header is insufficient for DoS protection as it can be spoofed or absent (chunked encoding). Active byte counting during stream processing is required. Furthermore, path-based access controls must be applied to the *normalized* path (after URI construction) to prevent attackers from using relative segments to reach unauthorized endpoints.
**Prevention:** Enforce request body size limits using both `Content-Length` validation and stream transformers to catch oversized payloads in all transfer modes. Always validate the path of the final, resolved `Uri` object rather than just the raw request URI to ensure traversal segments are correctly handled before authorization checks.
