...
class _DashboardScreenState extends State<_DashboardScreen> {
  ...
  Future<void> _loadData() async {
    ...
    for (int i = 0; i < historyLength; i++) {
      ...
      // BOLT: Calculate consumption delta (difference from chronologically previous reading).
      String? delta;
      if (i < historyLength - 1) {
        final currentVal = double.tryParse(c1.replaceAll(',', '.'));
        final previousVal = double.tryParse(history[i + 1].valorContador1.replaceAll(',', '.'));
        if (currentVal != null && previousVal != null && currentVal > previousVal) {
